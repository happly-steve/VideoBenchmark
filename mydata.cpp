...
int videortsp() {
    ...
    while (av_read_frame(format_ctx, &packet) >= 0) { //read ~ 1000 frames
        ...
        if (packet.stream_index == video_stream_index) {    //packet is video
            ...
            result +=
                    avcodec_decode_video2(codec_ctx, picture, &check, &packet);
//            std::cout << "Bytes decoded "
//                      << result
//                      << " check "
//                      << check
//                      << std::endl;
            if (cnt > 100)    //cnt < 0)
                ...
                std::ofstream fcnt;
                file << "Framecount.txt";
                fcnt.open(file.str().c_str());
                fcnt << "Frames:" << cnt << std::endl;
                fcnt << "Bytes decoded:" << result << std::endl;
                fcnt.close();
            }
            cnt++;
        }
